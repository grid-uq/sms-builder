...
@NamedQuery(name = RevisionQuery.REVISION_TOTAL_REFERENCIAS_TIPO_FUENTE_FILTRO, query = "select count(1) from Metadato m,Fuente f where m.referencia.revision.id = :id and m.referencia.filtro = :filtro and f.tipo = :tipoFuente and m.identifier = co.edu.utp.gia.sms.entidades.TipoMetadato.FUENTE and m.value = f.nombre ")
...
public class RevisionQuery extends Queries {
     ...
     * <code>select count(1) from Metadato m,Fuente f where m.referencia.revision.id = :id and f.tipo = :tipoFuente and m.identifier = co.edu.utp.gia.sms.entidades.TipoMetadato.FUENTE and m.value = f.nombre </code>
     */
    public static final String REVISION_TOTAL_REFERENCIAS_TIPO_FUENTE_FILTRO = "Revision.totalReferenciasTipoFuenteFiltro";
