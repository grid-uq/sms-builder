...
@Entity
@NamedQuery(name = ReferenciaQuery.REFERENCIA_GET_ALL, query = "select new co.edu.utp.gia.sms.dtos.ReferenciaDTO( r , revision.pasoSeleccionado.orden ) from Revision revision inner join revision.pasoSeleccionado.referencias r where revision.id = :idRevision ORDER BY r.spsid,r.nombre")
@NamedQuery(name = ReferenciaQuery.REFERENCIA_GET_ALL_DESTACADAS, query = "select new co.edu.utp.gia.sms.dtos.ReferenciaDTO( r ) from Revision revision inner join revision.pasoSeleccionado.referencias r where revision.id = :idRevision and r.relevancia is not null ORDER BY r.spsid,r.nombre")
@NamedQuery(name = ReferenciaQuery.REFERENCIA_GET_EVALUACION_ATRIBUTO_CALIDAD, query = "select new co.edu.utp.gia.sms.dtos.ReferenciaDTO( r ) from Revision revision inner join revision.pasoSeleccionado.referencias r inner join r.evaluacionCalidad e where revision.id = :idRevision and e.atributoCalidad.id = :idAtributoCalidad and e.evaluacionCualitativa = :valorEvaluacion  ORDER BY r.spsid,r.nombre")
@NamedQuery(name = ReferenciaQuery.REFERENCIA_GET_ATRIBUTO_CALIDAD, query = "select new co.edu.utp.gia.sms.dtos.ReferenciaDTO( r ) from Revision revision inner join revision.pasoSeleccionado.referencias r inner join r.evaluacionCalidad e where revision.id = :idRevision and e.atributoCalidad.id = :idAtributoCalidad  ORDER BY r.spsid,r.nombre")
...
@NamedQuery(name = ReferenciaQuery.REFERENCIA_SCIS, query = "select r.sci from Revision revision inner join revision.pasoSeleccionado.referencias r where revision.id = :idRevision")
